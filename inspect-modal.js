...
const targetIds = ['124:1168', '206:1062'];
const results = {};
...
function traverse(node, targetId, path, collection) {
  if (node.id === targetId || path !== '') {
    ...
    if (node.children) {
      node.children.forEach(child => traverse(child, targetId, newPath, collection));
    }
...
fs.writeFileSync('extracted-modals.json', JSON.stringify(results, null, 2));
console.log("Modals extracted to extracted-modals.json");
